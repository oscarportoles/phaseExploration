% It corrects and thresholds connectivity matrices at subject level. Then it computes the 
% golbal connectivity matrix from individual subjects. Plots the global connectivity 
% matrix with either the Power or the ITPC. Notice that you need dto selcet mannualy either 
% Power or ITPC 
...
pairs = combnk([1:nCh],2);        % all pairs of electrodes combinations [nComb, 2]
...
% choose here to plot ITPC or power in the background of head topographical plot
pltS = gPowInStg; % gPowInStg: for Power; gItpcStg: for ITPC
pltT = gPowInTra; % gPowInTra: for Power; gItpcTra: for ITPC
titVis = 'Power'; % figure title 'Power' or 'ITPC'
...
pathdata = '/Data_JB_AssoRec/DS100bp2_35/events/ForBumps_OnRes_no20/PhaseConectiv/';
...
info = {'Viusalization of thresholded connectivity matrices (dwPLI), power and ITPC',...
...
for fr = 1:nBand
    for bu = 1:nStg
        ...
        gPairT = [];
        gStrenT = [];
        thCo = 14;  % threshold connectivity consistency, best 14 ~ 70%

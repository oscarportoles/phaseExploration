% It epochs from stimuli onset to participant's response on band pass filtered data. The 
% epochs are choosen based on the labels of each event on the .TXT document (preious step), 
% the reaction time, the amplitude threshods on raw data. 
% Segments are the same for all frequency bands. New data is separeted by conditions and 
% freuency bands. Epochs of baseline are the same as epochs of post stimulus data
...
info = ['segment bandpass filtered data from stimulus onset to subjects response;' ...
    '100 Hz sampling freq.; filter 2 to 35 Hz and frequency bands;' ...
...
%     EEGG = eeg_checkset( EEGG );
    ...
    namesave = [snamesO(sj).name(1:idSj) 'PostStimEpochBands100.mat'];
...
%                         (trialCode == 5 & [EEG.event(idxEvnt(tr)).RT > newpaSD(1)] & [EEG.event(idxEvnt(tr)).RT < newpaSD(2)]),
                    ...
                    if not(any(any((dataTest > 80)))) && not(any(any((dataTest < -80)))),
                        x = vertcat(x, size(thetaEEG,1) + 1);   % first sample of this trial in x; initialy thetaEEG = []

...
load electrodes.mat
load varForBumpsOn_Res12575.mat subjects coeff10 latent10 data x y
...
for i = 6:6
    if flagTime,
        filename = ['iniCondOut125_0530_' num2str(i) '_Bu.mat'];
...
for i = 1:8,
    ...
    for bump = 1:colB,
        figure(i)
        subplot(1, colB, bump)
        topoplot(electrodes5(bump,:), locElect, 'maplimits', [-12 12],'style', 'map', 'electrodes','off', 'shading', 'interp');
